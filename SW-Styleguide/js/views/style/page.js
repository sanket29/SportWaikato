...
function($, _, Backbone, marked, stylePageTemplate, config, jscssp, Pagedown, hljs, parseuri){
	...
	var StylePage = Backbone.View.extend({
		...
		render: function () {
			...
			// else if ( this.options.style.substr(0,7) === 'http://' ) {
			//     this.options.style = this.options.style.substr( this.options.style.lastIndexOf('/')+1 );
			// }
			...
			require(['text!'+ styleUrl], function (stylesheet) {
				...
				if (result == null) {
					result = [styleUrl, that.options.style, 'css'];
				}
